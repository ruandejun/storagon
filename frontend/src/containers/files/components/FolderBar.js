    ...
    return (
      ...
      >
        <ListItem button className="folderTitle" >
            {item.children.length > 0 && <FontAwesomeIcon style={{marginRight: 5}} size='sm' icon='chevron-down'/>}
            <span className="titleWrap"  onClick={mouseClick}>
                {isActive && item.children ? <FontAwesomeIcon icon='folder-open'/> : <FontAwesomeIcon icon='folder'/> }
