...
app.controller('accountController', ['$scope', '$location', 'User_ClientAPI', '$cookies', function ($scope, $location, $sdk, $cookies) {
    ...
    $scope.resendActivationEmail = function (){
        ...
        $sdk.resendActivationEmail(function(){        
            ...
            $('#modal-activate-email p.modal-body').html("Successful! Please check your email to confirm");
            $('#modal-activate-email').foundation('reveal', 'open');
		}, function(){//error callback
			alert("Error Sending email");
			angular.element('.loader').hide();
            $('#modal-activate-email p.modal-body').html("Failed! Please check your email again!");
            $('#modal-activate-email').foundation('reveal', 'open');
			$scope.isProcessingResendActivationEmail=false;
		});
        $('a.close').on('click', function() {
            $('#modal-activate-email').foundation('reveal', 'close');
        });
    };

    $scope.applyAffiliate = function () {
      $('#modal-apply-affiliate').foundation('reveal', 'open');
    };
