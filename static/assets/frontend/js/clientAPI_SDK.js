...
function applyResumableToInput(inputID, submitID, progress_id, upspeed_id, folder_id, randomID, callback, errorCallback) {
  var r = new Resumable({
  ...
  r.on('fileAdded', function (file) {
    ...
    reader.onload = function (event) {
      ...
      r.key = generateEncryptionKey();//'000102030405060708090a0b0c0d0e0f';

      //init AES Encryptor
      //var key = CryptoJS.enc.Hex.parse(r.key);
      //var iv  = CryptoJS.enc.Hex.parse('00000000000000000000000000000001');
      //console.log("Init AES Encryptor with key="+key);
      //r.aesEncryptor = CryptoJS.algo.AES.createEncryptor(key, {mode: CryptoJS.mode.CTR, iv: iv });
      r.duplicated = false;

      createUploadSession(file_hash, file.size, file.fileName, folder_id, r.key, function (data) {
        //sucess callback
        //console.log(data);
        ...
        if (data.encryptResumeFileKey) { //resume using previous filekey
          r.key = Base64.decode(data.encryptResumeFileKey);
        }

        //init encryptor for resumable file;
        file.initEncryptor(r.key);
        //set ready
        uploadProgressPanel.find('tr.deactivate, tr#' + randomID).remove();
        uploadProgressPanel.append('<tr id=\'' + randomID + '\' class=\'initialize\'><td>' + file.fileName + '</td><td>' + filesize(file.size) + '</td><td class=\'progress-status\'><div class=\'progress large-12 success round\'><span class=\'meter\' id=\'' + progress_id + '\' style=\'width: 0%;\'></span></div></td><td id=\'' + upspeed_id + '\'></td><td><a href=\'javascript:void(0)\' class=\'stop-progress\'><i class=\'fi fi-stop size-14\'></i></a></td></tr>');
    ...
    $('#' + submitID).click(function (e) {
      $("#uploadfile").foundation('reveal', 'close');
  ...
  r.on('fileSuccess', function (file, message) {
    //save the auto generated realFileKey localy
    if (!r.duplicated)
      window.localStorage.setItem(StorageFileKeyPrefix + r.file_id, r.key);
  ...
  r.on('cancel', function () {
    $('tbody td#upspeed_' + randomID).addClass('alert');
    $('tbody td#upspeed_' + randomID).closest('tr').addClass('deactivate');
    $('tbody td#upspeed_' + randomID).html('Stopped');
    r = null;
  });
...
function getLink(file_id_list, callback) {
  ...
  $.ajax(add_auth_ajax({
    type: "GET",
    url: "/clapi/file/getLink/?" + postString,
    dataType: "json",
    success: function (json) {
      for (var i in json.download_url_list) {
        var url = json.download_url_list[i];
        var file_id = url.split('#')[0].split('/')[4];
        var fileKey = url.split('#')[1];
        if (fileKey)
          window.localStorage.setItem(StorageFileKeyPrefix + file_id, fileKey);
      }
      callback(json);
    }
  }));
